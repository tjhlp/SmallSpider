...
class WangYISpider(object):
    ...
    def run(self):
        tasks = []
        tasks.append(Process(target=self.get_url_list))
        for i in range(3):
            tasks.append(Process(target=self.get_html))
        for i in range(5):
            tasks.append(Process(target=self.get_item))
        tasks.append(Process(target=self.save_item))
